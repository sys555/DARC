class Graph:
    ...
    def init(cls, config):
        ...
        for node_cls in config['node']:
            if isinstance(node_cls, tuple):
                for _ in range(node_cls[1]):
                    ...
                    node_lookup[node_instance.__class__.__name__, id_counter] = node_instance
                    id_counter += 1
            else:
        ...
        for src, dest in config['edge']:
            graph.edges.append((node_lookup[src.__name__], node_lookup[dest.__name__]))
        
        return graph
    
    def find_type(self, type_name):
        return [node for node in self.nodes if node.__class__.__name__ == type_name]
...
class Task:
    ...
    def run(self):
        ...
        self.result = "expected task result"  # Modify this with actual execution logic
