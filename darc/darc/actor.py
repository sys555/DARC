...
class AbstractActor(pykka.ThreadingActor):
    def __init__(self):
        ...
        self.instance: Dict[str, pykka.ThreadingActor] = (
            dict()
        )  # addr -> pykka actor_ref
        self.message_box = []
    ...
    def send(self, message):
        if (
            message.to_agent in self.address_book
            and self.address_book[message.to_agent] in self.instance
        ):
