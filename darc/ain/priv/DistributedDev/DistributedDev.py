...
from darc.agent.llm.prompt.system_prompt_template import SYS_PROMPT_DICT
...
def compute(input: bytes) -> str:
    ...
    prompt = data["content"]
    response = get_answer_sync(prompt, SYS_PROMPT_DICT["Dev"])

    messages = [{
            "content": response,
            "parameters": {
                "from_role": "DistributedDev",
                "to_role": "DistributedTester",
            }
        }
    ]
